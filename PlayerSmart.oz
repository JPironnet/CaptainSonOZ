...
in
   ...
   fun{ChargeItem ID KindItem PlayerState}
	 ...
	 if (PlayerState.target.isTarget==false) then %if the player has launched his sonar but not his drone
	 ...
	 if (PlayerState.missileAmmo<2)
	    ...
	    if(PlayerState.mineCharge+1==Input.mine)
	       KindItem='mine'
   ...
   fun{FireItem ID KindFire PlayerState}
	 ...
	 if (PlayerState.target.isTarget==false) then %if the player has a target but does not have launched his drone
	    ...
	    if Manhattan>=2 then %pour que le missile explose pas sur lui
		  ...
		  KindFire=nil
		  {Print 'Le joueur n a pas tirer'}
		  NewPlayerState={AdjoinList PlayerState [droneAmmo#PlayerState.droneAmmo-1]}
		  NewPlayerStateNewPlayerState=PlayerState
		  NewPlayerState
	       end
	    end
	 end
   ...
   fun{FireMine ID Mine PlayerState}
      ...
      ID=PlayerState.id
      fun{FireMineLoop ID Mine PlayerState MineLocation}
	 case MineLocation of nil then
	    Mine=null
	    NewPlayerState=PlayerState
	    NewPlayerState
	 [] H|T then
	    Manhattan={Abs H.1.x-PlayerState.position.x} + {Abs H.1.y-PlayerState.position.y}
	    if Manhattan < 2 then
	       {FireMineLoop ID Mine PlayerState T}
	    else
	       Mine=H
   ...
   in
      {FireMineLoop ID Mine PlayerState PLayerState.mineLocation}
   end
   ...
   fun{SayMove ID Direction PlayerState}
	 ...
	 if Direction=='North' then
	    if {IsPositionOk PlayerState.target.position.x-1 PlayerState.target.position.y}==1 then
	       NewPosition=pt(PlayerState.target.position.x-1 PlayerState.target.position.y)
	 ...
	 elseif Direction=='West' then
	    if {IsPositionOk PlayerState.target.position.x PlayerState.target.position.y-1}==1 then
	       NewPosition=pt(PlayerState.target.position.x PlayerState.target.position.y-1)
	       NewTarget={AdjoinList PlayerState.target [position#NewPosition]}
	       NewPlayerState={AdjoinList PlayerState [target#NewTarget]}
	       NewPlayerState
	    else
...
%%%%%%%%%%% Useful functions %%%%%%%%%%%%%%%%%%%
   ...
   fun{IsVisited X Y List}
    ...
    fun{MoveRandomly ID Position Direction PlayerState}
      ...
      Dir={OS.rand} mod 5+1
      {Print 'Le joueur smart a choisi la direction aléatoire : '} 
      if Dir==2 then
